templates.overlay = function(data, params){
    ...
    if (params === 'resume' || params === 'result' || params === 'email'){
        ...
        if (params === 'result'){
            content += `<h3>${data.copy.content.result.title}</h3><p>${data.copy.content.result.content}</p><p>${data.copy.content.result.disclaimer}</p>`;
        ...
        content += `<div><input type="text" placeholder="${data.email.placeholder}" id="email-field"><button class="send">${data.email.cta}</button><p>${data.email.disclaimer}</p></div>`;
